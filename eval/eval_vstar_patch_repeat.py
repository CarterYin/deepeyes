...
parser.add_argument('--tool_patch_repeat', type=int, default=1, help='Repeat cropped patch N times in tool response')
parser.add_argument('--max_images_per_tool_response', type=int, default=2, help='Max images allowed in one tool response request')
parser.add_argument('--result_tag', type=str, default=None, help='Optional suffix tag for result file names')
parser.add_argument('--eval_model_name', type=str, default=None, help='Model name for evaluation')
parser.add_argument('--num_workers', type=int, default=8)
args = parser.parse_args()

if args.tool_patch_repeat < 1:
    raise ValueError("--tool_patch_repeat must be >= 1")
if args.max_images_per_tool_response < 1:
    raise ValueError("--max_images_per_tool_response must be >= 1")
...
result_tag = args.result_tag if args.result_tag else f"repeat{args.tool_patch_repeat}"
effective_patch_repeat = min(args.tool_patch_repeat, args.max_images_per_tool_response)
if effective_patch_repeat < args.tool_patch_repeat:
    print(
        f"[WARN] tool_patch_repeat={args.tool_patch_repeat} exceeds max_images_per_tool_response="
        f"{args.max_images_per_tool_response}. Fallback to {effective_patch_repeat}."
    )
...
def process(img_arg):
    ...
    try:
        while '</answer>' not in response_message:
            ...
            if start_token in response_message:
                ...
                zoom_filename = f"{img_stem}_turn{turn_idx:02d}_bbox_{_bbox_token(bbox_str)}.png"
                ...
                for _ in range(effective_patch_repeat):
                ...
                print_user_content = []
                for _ in range(effective_patch_repeat):
                    print_user_content.append({"type": "image_url", "image_url": {"url": "data:image/jpeg;base64,"}})
                print_user_content.append({"type": "text", "text": user_prompt})
                print_messages.extend([
                    {"role": "assistant", "content": response_message},
                    {"role": "user", "content": print_user_content},
                ])
                turn_idx += 1
            else:
    ...
    save_info = {
        ...
        'tool_patch_repeat': args.tool_patch_repeat,
        'effective_patch_repeat': effective_patch_repeat,
        'status': status,
    }
    return save_info
...
if __name__ == "__main__":
    ...
    for test_type in test_types:
        save_name = f"result_{test_type}_{args.model_name}_{result_tag}.jsonl"
        save_json = []
