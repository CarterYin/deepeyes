...
parser.add_argument('--result_tag', type=str, default='bbox_only', help='Result file suffix tag')
...
if __name__ == '__main__':
    ...
    final_acc = {}
    for test_type in test_types:
        type_acc = np.mean(per_type_acc[test_type]) * 100
        print(f"Accuracy for {test_type}: {type_acc:.2f}%")
        final_acc[test_type] = type_acc
    overall_acc = np.mean(all_acc) * 100
    print(f"Overall Accuracy: {overall_acc:.2f}%")
    final_acc['overall'] = overall_acc
    final_acc['error_nums'] = error_nums_type
    final_acc['error_preds'] = error_preds

    final_name = f'final_acc_{args.result_tag}.json'
    with open(os.path.join(result_root_path, final_name), 'w') as f:
